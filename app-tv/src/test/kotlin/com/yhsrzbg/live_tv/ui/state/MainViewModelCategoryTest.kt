...
class MainViewModelCategoryTest {
    @Test
    fun categoryEntries_fallsBackToTopCategoryWhenChildrenMissing() = runTest {
        val fakeSite = ViewModelCategoryFakeSite(
            categories = listOf(
                LiveCategory(
                    id = "top",
                    name = "Top",
        ...
        val entries = viewModel.categoryEntries("fake")

        assertEquals(1, entries.size)
        assertEquals("top", entries.first().id)
        assertEquals("top", entries.first().parentId)
        database.close()
    }
}

private class ViewModelCategoryFakeSite(
    ...
    override suspend fun categories(): List<LiveCategory> = categories
