...
class SearchAnchorRepositoryTest {
    @Test
    fun searchAnchors_returnsAnchorItems() = runTest {
        val expected = listOf(
            LiveAnchorItem(
                roomId = "r-1",
                avatar = "a",
        ...
        val fakeSite = FakeAnchorSite(result = expected)
        ...
        val result = repository.searchAnchors(
            siteId = "fake",
            keyword = "abc",
            page = 3,
        )
        ...
        assertEquals("abc", fakeSite.lastKeyword)
        assertEquals(3, fakeSite.lastPage)
        database.close()
    }
}

private class FakeAnchorSite(
    private val result: List<LiveAnchorItem>,
) : LiveSite {
    ...
    override suspend fun searchAnchors(keyword: String, page: Int): LiveSearchAnchorResult {
        lastKeyword = keyword
        lastPage = page
        return LiveSearchAnchorResult(false, result)
    }
